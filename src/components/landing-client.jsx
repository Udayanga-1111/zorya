...
export function LandingClient({ isLoggedIn }) {
    ...
    <div ref={containerRef} className="flex flex-col min-h-screen">
      ...
      <header className="fixed top-0 w-full px-6 py-4 flex items-center justify-between z-50 glass-nav">
        ...
        <div className="flex items-center gap-4">
          {isLoggedIn ? (
            <Link href="/dashboard">
              <Button variant="ghost" className="hidden sm:inline-flex btn-magnetic">Dashboard</Button>
            </Link>
          ) : (
            <Link href="/login">
      ...
      <main className="flex-1 flex flex-col items-center relative z-10 pt-20">
        ...
        <section className="w-full h-[90vh] px-6 flex flex-col items-center justify-center text-center max-w-4xl mx-auto">
          <div ref={heroTextRef} className="flex flex-col items-center pointer-events-none">
            ...
            <div className="flex flex-col sm:flex-row gap-6 w-full sm:w-auto pointer-events-auto justify-center">
              {isLoggedIn ? (
                <Link href="/dashboard" className="w-full sm:w-auto">
                  <Button size="lg" className="w-full rounded-full btn-magnetic btn-glow h-14 px-8 text-lg">Go to Dashboard</Button>
                </Link>
              ) : (
                <>
